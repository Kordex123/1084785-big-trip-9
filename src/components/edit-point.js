...
import {Activities, Events, Transfers} from "./point-data";
import {deepCopy} from "./utils/object-utils";
import {PointMode} from "./dict";

export default class EditPoint extends AbstractComponent {
  constructor({startDate, endDate, type, destination, price, additionalOptions, isFavorite}, offersDict, destinationDict, mode = PointMode.EDIT) {
    super();
    ...
    this._SHAKE_DURATION_MILLIS = 1200;
  ...
  getTemplate() {
    ...
    const destinationValidationPattern = this._destinationDict.map(({name}) => name).join(`|`);
    const destinationValidationMessage = `Please choose from dropdown values: ${this._destinationDict.map(({name}) => name).join(`, `)}`;

    return `
      <form class="${this._mode === PointMode.ADD ? `trip-events__item` : ``} event  event--edit" action="#" method="post">
        <header class="event__header">
          ...
          <div class="event__field-group  event__field-group--destination">
            ...
            name="event-destination" value="${this._destination.name}" list="destination-list-1"
            pattern="${destinationValidationPattern}" title="${destinationValidationMessage}">
          ...
          <div class="event__field-group  event__field-group--price">
            ...
            <input class="event__input  event__input--price" id="event-price-1" type="number" name="event-price" value="${this._price}" min="0">
          </div>

          <button class="event__save-btn  btn  btn--blue" type="submit">Save</button>
          <button class="event__reset-btn" type="reset">${this._mode === PointMode.ADD ? `Cancel` : `Delete`}</button>

          ${this._mode !== PointMode.ADD ? `
  ...
  _shake() {
    ...
    this._element.classList.add(`error-border`);
    setTimeout(() => {
      this._element.classList.remove(`shake`);
    }, this._SHAKE_DURATION_MILLIS);
  }

  block(buttonType) {
    const saveButton = this._element.querySelector(`.event__save-btn`);
    const deleteButton = this._element.querySelector(`.event__reset-btn`);
    ...
    if (buttonType === `save`) {
      ...
      deleteButton.textContent = this._mode === PointMode.ADD ? `Canceling...` : `Deleting...`;
    }
  }
  ...
  _unblock() {
    ...
    deleteButton.textContent = this._mode === PointMode.ADD ? `Cancel` : `Delete`;
  }
